...
public class ChatListener {
    ...
    @SubscribeEvent
    public void onClientChat(ClientChatEvent event) throws FileNotFoundException, ExecutionException, InterruptedException {
        ...
        if (message.startsWith(".f1")) {
            ...
            filter.writeToFile(MsgList.DONATE_CHAT);
        }
        if (message.startsWith(".f2")) {
            event.setCanceled(true);
            filter.writeToFile(MsgList.CREATIVE_CHAT);
        }
        if (message.startsWith(".f3")) {
            event.setCanceled(true);
            filter.writeToFile(MsgList.PM);
        }
        if(message.startsWith(".test")) {
            event.setCanceled(true);
            CompletableFuture<List<Message>> completableFuture = CompletableFuture.supplyAsync(() -> {
                try {
                    return Func.setMessagesFromGitHub();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            });
            System.out.println(completableFuture.get());
//            Callable<List<Message>> call = Func::setMessagesFromGitHub;
//            FutureTask<List<Message>> futureTask = new FutureTask<>(call);
...
//                if (Configs.Filters.IgnoreList.length==0) {
//                    mc.player.sendMessage(new TextComponentString(prefix+"Список заблокированных игроков пуст. Добавьте игроков, используя §7.ignore [ник]"));
//                    return;
//                }
//                for (String player:Configs.Filters.IgnoreList) {
//                    component.appendSibling(new TextComponentString("\n §7"+player).setStyle(new Style().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString("§7Нажмите, чтобы убрать игрока"))).setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, ".ignore "+player))));
//                }
//                mc.player.sendMessage(new TextComponentString("\n"+prefix+"Список заблокированных игроков:\n").appendSibling(component).appendText("\n\n §fНажмите на игрока, чтобы убрать его"));
//            }
//            else {
//                int length = Configs.Filters.IgnoreList.length;
//                Func.editIgnoreList(new String[]{"split[1]"});
//                mc.player.sendMessage(new TextComponentString(prefix+"Теперь в списке заблокированных игроков §7"+Configs.Filters.IgnoreList.length+" §fигроков. (ранее: "+length+")"));
//            }
//        }
        switch (message) {
